...
rem run necessary tmp scripts
  ...
  rem unzip
    ...
    if not "%PAOperation%"=="list" goto aunzipend
      ...
      : aunzipend
  rem adf
    if "%ADFING%"=="true" goto adfend
    if "%ADFING%"=="" goto adfstart
      : adfstart
      if exist "C:\workspace\buildtemp\files\bat\adftmp.bat" call adftmp
      if exist "C:\workspace\buildtemp\files\bat\adftmp.bat" call bdel adftmp
      : adfend
